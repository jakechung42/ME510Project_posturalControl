function [Amplitude_save,Amplitude_out_m,Amplitude_out_stdev,Amplitude_in_m,Amplitude_in_stdev,phase_save,Amplitude_ratio_stdev] = Auto_Bode_Tiff(X,Y,N_test_cycles,N_points_per_cycle,N_mean_percent,N_initial_discard_percent)
...
%           plot(iymin,ymin,'og')
    ...
    Amplitude_in_vec = Amplitude_in((mean_start):(mean_end));
    Amplitude_in   = sort(Amplitude_in);
    Amplitude_in_m = mean(Amplitude_in((mean_start):(mean_end)));
    Amplitude_in_stdev = std(Amplitude_in((mean_start):(mean_end)));
    ...
    Amplitude_out_stdev = std(Amplitude_out((mean_start):(mean_end)));
