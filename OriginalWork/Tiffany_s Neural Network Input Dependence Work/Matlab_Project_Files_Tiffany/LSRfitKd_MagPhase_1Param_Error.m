function [KdFitValue,stdev] = LSRfitKd_MagPhase_1Param_Error(Kdstart,Kdend,divs,freq_Hz,magRatioData,phaseData)
...
% tau = [0.004 1000];
% Kp = linspace(0.5, 2, 15);
% Kp = [1.258 100];
Kd = linspace(Kdstart, Kdend, divs);
% Kd = [0.3 100];
...
%             oltf = delay*lowFiltNeuron^4*(Kp(k2) + Kd(k3)*s*lowFiltDerivative);
            oltf = lowFiltNeuron^2*Kd(k3)*s*lowFiltDerivative;
            ...
            errors(k3) = mag_weight*mag_error + phase_weight*phase_error;
...
oltfFit = lowFiltNeuron^2*KdFitValue*s*lowFiltDerivative;
